...
## v2.7 fixes (2026-09-17)
- Fixed CFM KV loading by importing `dp` before using `dp(...)` expressions.
- Added `show_info_dialog()` so CFM/DWPT/Monthly error handling cannot crash with a missing method.
- Renamed Timesheet Approval routing to Backstop/Line Manager.
- Added Backstop/Line Manager selection to Leave Management and saved it with leave requests.
- Leave Excel export now uses the official ROHI Leave Management template uploaded for this release, preserving its form layout and formatting.
