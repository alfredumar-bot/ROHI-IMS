...
def _http_upload_excel(filepath, endpoint, report_type, state_office=""):
    """Upload a report file to a configured server/Apps-Script endpoint.
    ...
    try:
        ...
        body.extend(
            ...
            f"Content-Type: {'application/vnd.openxmlformats-officedocument.wordprocessingml.document' if filename.lower().endswith('.docx') else 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'}\r\n\r\n"
            .encode("utf-8")
        )
        body.extend(payload)
        body.extend(f"\r\n--{boundary}--\r\n".encode("utf-8"))
...
# Must run before any kivy.core.window import - this app's UI is flat
# cards/text with no diagonal/curved graphics that benefit visibly from
# antialiasing, but MSAA still costs real per-frame GPU time on every
# device, and that cost is disproportionately painful on the budget
# Android GPUs this app is actually deployed on (a common, well-known
# source of scroll/render jank in Kivy apps). Turning it off is free
# speed with no visible downside for this UI.
...
class ROHIAttendanceApp(MDApp):
    ...
    def build(self):
        ...
        # Show the login screen immediately after the first frame. Network
        # checks and non-essential synchronization continue after it is shown.
        Clock.schedule_once(self._restore_or_open_login, 0.05)
        ...
        Clock.schedule_once(lambda dt: self._excel_sync_schedule_tick(), 5)
        ...
        Clock.schedule_once(lambda dt: self._sync_cfm_statuses_from_google(), 8)
        ...
        Clock.schedule_once(lambda dt: self._auto_sync_tick(), 6)
        ...
        Clock.schedule_once(lambda dt: self._auto_connect_excel_links(), 8)
    ...
    def on_start(self):
        ...
        # Starting Android's companion service can briefly block on slower
        # phones. Defer it until after the login UI has been painted.
        Clock.schedule_once(lambda dt: self._start_background_reminder_service(), 5)
    ...
    def show_info_dialog(self, title, message):
        """Show a safe informational dialog used by dashboard/report screens.

        This helper is intentionally self-contained so a screen-loading error
        never causes a second AttributeError while trying to report the first
        error (as happened with the CFM screen).
        """
        try:
            dialog = MDDialog(
                title=str(title or "Information"),
                text=str(message or ""),
            ...
            logger.exception("Unable to display information dialog")
            return None
        ...
        try:
            ...
            if email_val:
                ...
                if os.path.exists(db_path):
                    ...
                    # Single pass over the same rows computes present/late/absent
                    # together - this used to run the identical query twice.
                    # Kept as two independent sets (not mutually exclusive),
                    # matching the original two-query behavior exactly: a day
                    # counts as "present" whenever it has a check-in, and
                    # separately counts as "absent" whenever its status says
                    # so - the same row can contribute to both, as before.
                    present_days_seen = set()
                    absent_days_seen = set()
                    for check_in_time, status in rows:
                        ...
                        if status and "Late" in status:
                            days_late += 1
                        if status and "absent" in str(status).lower():
                            absent_days_seen.add(day_key)
                    days_present = len(present_days_seen)
                    days_absent = len(absent_days_seen)
    ...
    def _export_timesheet_template(self, for_sync=False, directory=None):
        ...
        # Clear the template's fixed 31-row area first. Months with 28-30
        # days must not retain a date from the previous selected month.
        ...
        # Populate only the dates belonging to the selected month.
        ...
        selected_section = getattr(ids, "timesheet_section_spinner", None)
        selected_officer = getattr(ids, "timesheet_officer_spinner", None)
        section_text = selected_section.text if selected_section else ""
        officer_text = selected_officer.text if selected_officer else ""
        if officer_text and officer_text != "Select officer":
            ws["A49"] = f"Supervisor Signature ({officer_text}): ____________________________"
            ws["F49"] = f"Section: {section_text}"
        else:
    ...
    def email_dwpt_report(self):
        ...
        if not path or not os.path.exists(path) or "dwpt" not in os.path.basename(path).lower():
            path = os.path.join(APP_DIR, "templates", "Daily Weekly Performance Tracker report.docx")
    ...
    def email_monthly_report(self):
        ...
        if not path or not os.path.exists(path) or "monthly" not in os.path.basename(path).lower():
            path = os.path.join(APP_DIR, "templates", "monthly report.docx")
    ...
    def _create_word_report(self, template_name, output_name, fields):
        """Create a Word report from the supplied ROHI Word template.

        The templates remain regular .docx files. Submitted values are added
        as a clean completion section so the report opens in Word and keeps
        the supplied ROHI activity-report wording.
        """
        template_path = os.path.join(APP_DIR, "templates", template_name)
        if not os.path.exists(template_path):
            raise FileNotFoundError(f"Word template missing: {template_name}")
        output_path = os.path.join(self._get_download_dir(), output_name)
        ns = "http://schemas.openxmlformats.org/wordprocessingml/2006/main"
        with zipfile.ZipFile(template_path, "r") as source:
            document_xml = source.read("word/document.xml").decode("utf-8")
            body_end = "</w:body>"
            if body_end not in document_xml:
                raise ValueError("The Word template has no document body.")
            paragraphs = []
            for label, value in fields:
                safe_label = (str(label).replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;"))
                safe_value = (str(value or "-").replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;"))
                paragraphs.append(
                    f'<w:p><w:r><w:rPr><w:b/></w:rPr><w:t>{safe_label}: </w:t></w:r>'
                    f'<w:r><w:t xml:space="preserve">{safe_value}</w:t></w:r></w:p>'
                )
            completion = '<w:p><w:r><w:rPr><w:b/></w:rPr><w:t>Completed Report Details</w:t></w:r></w:p>' + ''.join(paragraphs)
            document_xml = document_xml.replace(body_end, completion + body_end)
            with zipfile.ZipFile(output_path, "w", zipfile.ZIP_DEFLATED) as target:
                for item in source.infolist():
                    target.writestr(item, document_xml.encode("utf-8") if item.filename == "word/document.xml" else source.read(item.filename))
        return output_path

    def _report_upload(self, kind, fields, filename):
        """Create a Word report and upload it without blocking the UI."""
        ...
        def worker():
            try:
                template_name = "monthly report.docx" if kind == "monthly" else "Daily Weekly Performance Tracker report.docx"
                path = self._create_word_report(template_name, filename, fields)
    ...
    def open_cfm(self):
        ...
        try:
            ids = self.cfm_screen.ids
            if not ids or "cfm_message" not in ids:
                raise RuntimeError("CFM layout did not load; check the app log for cfm.kv.")
            ...
            self.show_info_dialog("CFM", "The Complaints & Feedback screen could not load. Please reopen the app; its diagnostic details are saved in the app log.")
    ...
    def submit_leave_request(self):
        ...
        try:
            create_leave_request({
                ...
                "days": days,
                "reason": reason,
                "manager_email": (ids.leave_manager_spinner.text.strip()
                                  if hasattr(ids, "leave_manager_spinner") else ""),
    ...
    def export_leave_excel(self, directory=None, silent=False):
        """Export the current leave application using the official ROHI template.

        The uploaded ROHI Leave Management template is used as the workbook
        itself, preserving its merged cells, borders, fonts, widths, logo and
        section layout. The latest leave request is populated into that form.
        """
        try:
            if not self.current_user:
                return None
            ...
            staff_id = str(self.current_user[18]) if len(self.current_user) > 18 and self.current_user[18] else "-"
            department = str(self.current_user[15]) if len(self.current_user) > 15 and self.current_user[15] else "-"
            position = str(self.current_user[17]) if len(self.current_user) > 17 and self.current_user[17] else "-"
            state_office = str(self.current_user[13]) if len(self.current_user) > 13 and self.current_user[13] else "-"
            manager = ""
            if hasattr(self.leave_screen.ids, "leave_manager_spinner"):
                manager = self.leave_screen.ids.leave_manager_spinner.text.strip()
            if not manager or manager == "Select Backstop/Line Manager":
                manager = "-"

            template_candidates = [
                os.path.join(APP_DIR, "templates", "ROHI_Leave_Management_Official_Template.xlsx"),
                os.path.join(APP_DIR, "templates", "ROHI Leave Management Official Template.xlsx"),
            ]
            template_path = next((p for p in template_candidates if os.path.exists(p)), None)
            if not template_path:
                raise FileNotFoundError("Official ROHI Leave Management Excel template is missing from the app package.")

            from openpyxl import load_workbook
            wb = load_workbook(template_path)
            ws = wb["Leave Application"] if "Leave Application" in wb.sheetnames else wb.active

            # Keep the official wording/layout, but update the routing label requested by ROHI.
            ws["E7"] = "Backstop/Line Manager"
            # Staff information
            ws["B6"] = fullname
            ws["E6"] = staff_id
            ws["H6"] = department
            ws["B7"] = position
            ws["E7"] = manager
            # Preserve the label while placing the selected manager in the merged field.
            # The template label is in E7, so move the label to D7 and use E7:F7 for the value.
            ws["D7"] = "Backstop/Line Manager"
            ws["E7"] = manager
            ws["H7"] = state_office

            if rows:
                row = rows[0]
                leave_type, start_date, end_date, days, reason, status = row[1], row[2], row[3], row[4], row[5], row[6]
                row_manager = row[9] if len(row) > 9 else ""
                if row_manager and row_manager != "-":
                    ws["E7"] = row_manager
                ws["B10"] = leave_type or "-"
                ws["E10"] = row[8] or datetime.now().strftime("%Y-%m-%d")
                try:
                    end_dt = datetime.strptime(str(end_date), "%Y-%m-%d")
                    ws["H10"] = end_dt.strftime("%Y-%m-%d")
                except Exception:
                    ws["H10"] = end_date or "-"
                ws["B11"] = start_date or "-"
                ws["E11"] = end_date or "-"
                ws["H11"] = days or 0
                ws["B12"] = reason or "-"
                ws["B14"] = "-"
                ws["B18"] = status or "Pending"
                ws["E18"] = "-"
                ws["H18"] = "Pending"
                ws["B19"] = row[7] or "-"
                ws["H19"] = "-"
                ws["B21"] = status or "Pending"
                ws["E21"] = manager
                ws["H21"] = row[8] or "-"
            else:
                ws["B10"] = "-"
                ...
                ws["H10"] = "-"
                ws["B11"] = "-"; ws["E11"] = "-"; ws["H11"] = 0
                ws["B12"] = "No leave request submitted."
                ws["B14"] = "-"
                ws["B18"] = "Pending"; ws["E18"] = "-"; ws["H18"] = "Pending"
                ws["B19"] = "-"; ws["H19"] = "-"
                ws["B21"] = "Pending"; ws["E21"] = manager; ws["H21"] = "-"

            # The official template has B7:C7 and E7:F7 merged, so use D7 as the
            # visible label and E7:F7 as the actual selected manager value.
            # Restore D7 as the label and preserve the template's formatting.
            ws["D7"] = "Backstop/Line Manager"
            ...
            # Protect the form from accidental edits while keeping the official layout intact.
            ws.protection.sheet = True
            ws.protection.password = REPORT_EXPORT_PASSWORD
            wb.save(filepath)
            self._encrypt_xlsx_open_password(filepath, REPORT_EXPORT_PASSWORD)
            ...
            logger.info("Official ROHI Leave Management report exported to %s", filepath)
            return filepath
        except Exception as exc:
            logger.exception("Error exporting official leave report")
    ...
    def send_timesheet_to_excel(self):
        ...
        """
        ids = self.timesheet_screen.ids
        officer = getattr(ids, "timesheet_officer_spinner", None)
        section = getattr(ids, "timesheet_section_spinner", None)
        if not officer or officer.text == "Select officer":
            self._set_timesheet_send_status("Select a section and officer before sending.", False)
            return
        try:
        ...
        if not path or not os.path.exists(path):
            ...
            return
        # The planned Windows approval app reads these queue records (or the
        # same payload from its service endpoint), signs, then uploads the
        # approved file to Google Drive. Keeping this JSON schema in the APK
        # makes the future app integration explicit and testable.
        self._queue_timesheet_approval(path, section.text, officer.text)
        self._send_excel_now(path, "timesheet", self._set_timesheet_send_status, cleanup_path=path)

    def _queue_timesheet_approval(self, filepath, section, officer):
        queue_path = os.path.join(APP_DIR, "timesheet_approval_queue.json")
        try:
            queue = []
            ...
            if not isinstance(queue, list):
                queue = []
            queue.append({
                "schema_version": 1, "status": "awaiting_officer_signature",
                "created_at": datetime.now().isoformat(timespec="seconds"),
                "employee_name": str(self.current_user[1] if self.current_user else ""),
                "employee_email": str(self.current_user[20] if self.current_user and len(self.current_user) > 20 else ""),
                "section": section, "assigned_officer": officer,
                "timesheet_file": os.path.basename(filepath),
            })
            with open(queue_path, "w", encoding="utf-8") as f:
                json.dump(queue, f, indent=2)
        except Exception:
            logger.exception("Could not queue timesheet approval metadata.")
    ...
    def _pay_period_days(year, month):
        """Return only dates within the selected calendar month.

        A selected September timesheet must not include August dates. This
        also makes exported Pay Period Date values match the month displayed
        in the month selector.
        """
    ...
    def generate_timesheet(self):
        """Build the daily grid only for the selected calendar month."""
        try:
            if not self.current_user:
                return
            ids = self.timesheet_screen.ids
    ...
    def update_leave_manager_options(self, value=None):
        """Populate the Leave Management Backstop/Line Manager dropdown.

        The choices mirror the routing officers used by Timesheet and also
        cover the programme/support sections used by ROHI.
        """
        choices = [
            "Select Backstop/Line Manager",
            "MEAL Officer", "Protection Officer", "Education Officer",
            "Livelihood Officer", "WASH Officer", "Nutrition Officer",
            "GBV Officer", "Communication Officer", "Information Management Officer",
            "HR Officer", "Finance Officer", "Logistics Officer",
            "Maintenance Officer", "Security Officer",
        ]
        try:
            spinner = self.leave_screen.ids.leave_manager_spinner
            spinner.values = choices
            if not spinner.text or spinner.text not in choices:
                spinner.text = choices[0]
        except Exception:
            logger.exception("Unable to update Leave Backstop/Line Manager choices")

    def update_timesheet_officer_options(self, section):
        """Populate routing officers without changing registration data."""
        choices = {
            "Finance": ["Finance Officer"], "MEAL": ["MEAL Officer"],
            "HR": ["HR Officer"], "SECURITY": ["Security Officer"],
        }
        try:
            spinner = self.timesheet_screen.ids.timesheet_officer_spinner
            spinner.values = choices.get(section, [f"{section.title()} Officer"] if section and section != "Select section" else [])
            spinner.text = "Select officer"
        except Exception:
            logger.exception("Unable to update timesheet officer choices.")
