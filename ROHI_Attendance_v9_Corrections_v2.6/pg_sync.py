...
def _open_connection(config):
    ...
    timeout=10 caps how long the underlying TCP connect can block. Without
    it, an unreachable host (wrong address, firewall silently dropping
    packets, server down) can hang for minutes at the OS level before
    failing, which made "Connecting..." on Settings look permanently stuck
    rather than actually failing with a message.
    """
