...
def create_table():
        ...
        for column, col_type in migrations:
            try:
                ...
                if "duplicate column" not in str(exc).lower():
                    raise
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_staff_synced ON staff(synced)")
        # Login (main.py verify_login) and every duplicate-check during
        # registration filter on LOWER(TRIM(email)) / LOWER(TRIM(staff_number)),
        # never on the raw column - a plain index on email/staff_number is
        # invisible to those queries since the column is wrapped in a
        # function. These expression indexes match the actual query shape,
        # so login (the single most frequent operation in the app, hit on
        # every launch) can use an index instead of a full table scan.
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_staff_email_norm ON staff(LOWER(TRIM(email)))")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_staff_number_norm ON staff(LOWER(TRIM(staff_number)))")
        conn.commit()
    finally:
        conn.close()
    ...
    create_leave_table()
    create_timesheet_approval_table()
    create_cfm_table()
    ...
    try:
        ...
        for column, col_type in (
            ...
            try:
                ...
                if "duplicate column" not in str(exc).lower():
                    raise
        # Indexes: this table grows by design (new rows every working day,
        # forever) and every dashboard stat, history view, and background
        # sync query filters on these columns - without an index each of
        # those was a full table scan, getting slower every month as the
        # table grows. Composite (email, check_in_time) covers the common
        # "this staff member, this month" pattern used throughout the app;
        # the synced/gform_synced indexes cover the background sync queries.
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_attendance_email_checkin ON attendance(email, check_in_time)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_attendance_synced ON attendance(synced)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_attendance_gform_synced ON attendance(gform_synced)")
        conn.commit()
    finally:
        conn.close()
...
def create_cfm_table():
    ...
    try:
        ...
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_cfm_cases_status ON cfm_cases(status)")
        conn.commit()
    finally:
        conn.close()
...
def create_leave_table():
    ...
    try:
        ...
        """)
        conn.execute("CREATE INDEX IF NOT EXISTS idx_leave_requests_staff_email ON leave_requests(staff_email)")
        conn.execute("CREATE INDEX IF NOT EXISTS idx_leave_requests_staff_email_norm ON leave_requests(LOWER(TRIM(staff_email)))")
        conn.execute("CREATE INDEX IF NOT EXISTS idx_leave_requests_synced ON leave_requests(synced)")
        existing = {row[1] for row in conn.execute("PRAGMA table_info(leave_requests)").fetchall()}
        if "handover" not in existing:
            conn.execute("ALTER TABLE leave_requests ADD COLUMN handover TEXT")
        conn.commit()
    finally:
        conn.close()


def create_timesheet_approval_table():
    """Store the employee-visible status for each submitted timesheet."""
    conn = _connect()
    try:
        conn.execute("""
        ...
        """)
        conn.execute("CREATE INDEX IF NOT EXISTS idx_timesheet_approvals_staff ON timesheet_approvals(LOWER(TRIM(staff_email)), submitted_at)")
        conn.commit()
    finally:
        conn.close()


def create_timesheet_approval(data):
    def operation(cursor):
        cursor.execute("""
            INSERT INTO timesheet_approvals (
                staff_email, staff_name, period, approver_role, approver_name,
                status, submitted_at
            ) VALUES (
                :staff_email, :staff_name, :period, :approver_role, :approver_name,
...
def get_timesheet_approval_counts(staff_email):
    conn = _connect()
    try:
        rows = conn.execute("""
            SELECT status, COUNT(*) FROM timesheet_approvals
            WHERE LOWER(TRIM(staff_email)) = LOWER(TRIM(?))
            GROUP BY status
        """, (staff_email or "",)).fetchall()
        counts = {"Pending": 0, "Approved": 0, "Not Approved": 0}
        for status, count in rows:
            status = str(status or "Pending").strip().title()
            if status == "Rejected":
                status = "Not Approved"
            if status in counts:
    ...
    def operation(cursor):
        cursor.execute("""
            INSERT INTO leave_requests (
                ...
                days, reason, handover, status, manager_email, submitted_at, synced
            ) VALUES (
                :staff_email, :staff_name, :leave_type, :start_date, :end_date,
                :days, :reason, :handover, 'Pending', :manager_email, :submitted_at, 0
            )
        """, data)
        return cursor.lastrowid
    return _run_write(operation)
...
def get_latest_leave_request(staff_email):
    """Return the newest request with approval-routing details for the official form."""
    conn = _connect()
    try:
        return conn.execute("""
            SELECT id, staff_name, leave_type, start_date, end_date, days, reason,
                   handover, status, manager_email, manager_comment, submitted_at, reviewed_at
            FROM leave_requests
            WHERE LOWER(TRIM(staff_email)) = LOWER(TRIM(?))
            ORDER BY id DESC LIMIT 1
        """, (staff_email or "",)).fetchone()
    finally:
        conn.close()
